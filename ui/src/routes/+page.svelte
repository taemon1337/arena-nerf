<script>
  ...
  import { currentStats, pollStats } from '$lib/api'
...
<div class="grid grid-rows-3 grid-cols-12 gap-4">
  <div class="row-span-3 col-span-3">
    ...
    {#each Object.entries($currentStats.scoreboard || {}) as [team, count]}
    ...
    {#each Object.entries($currentStats.nodeboard || {}) as [node, count]}
    ...
    <p>Started {$currentStats.start_at}</p>
    {#if $currentStats.completed}
    <p>Ended {$currentStats.end_at}</p>
    {:else}
    <p>In progress</p>
    {/if}
  </div>
  ...
  <div class="col-span-6 grid-rows-3">
    <Heading tag="h1" class="mb-4 dark:text-gray-400" customSize="text-4xl font-extrabold  md:text-5xl lg:text-6xl">
      Current Game Stats
      <Badge color="green">{$currentStats.status}</Badge>
    </Heading>
    <Table striped={true}>
      <TableHead>
  ...
  <div class="row-span-2 col-span-3">
    <Table noborder={true} shadow striped={true}>
      ...
      <TableBody>
        {#each [...($currentStats.events || [])].reverse() as event}
          <TableBodyRow>
