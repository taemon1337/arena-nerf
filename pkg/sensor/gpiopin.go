...
func (s *Sensor) ParseGpioPin(pinstr string) (int, error) {
  if pinstr == "" {
    return 0, ERR_EMPTY_PIN
  }

  switch s.conf.Device {
    case "opi", "orangepi", "orangepi3", "orangepi3zero":
      return s.ParseOrangePiGpioPin(pinstr)
    case "rpi", "raspberrypi", "raspberrypi4", "raspberrypi4zero":
      return s.ParseRaspberryPiGpioPin(pinstr)
    default:
      return 0, ERR_UNSUPPORTED_DEVICE
  }
}

func (s *Sensor) ParseOrangePiGpioPin(pinstr string) (int, error) {
...
func (s *Sensor) ParseRaspberryPiGpioPin(pinstr string) (int, error) {
  pinstr = strings.ToLower(pinstr)
